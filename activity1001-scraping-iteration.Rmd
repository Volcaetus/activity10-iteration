...
scrape_tmdb <- function(url){
  ...
  rating=
    page %>%
    html_nodes("#media_v4 .content_score") %>%
    html_element('p') %>%
    html_text() %>%
    as.numeric() %>% 
    .[1]
...
```{r}
url = "https://www.themoviedb.org/tv/top-rated?page=1"

page=read_html(url)

tmdb_url=page %>%
  html_nodes("#page_1 .content a") %>%
  html_attr("href")

tmdb_tv=paste0("https://www.themoviedb.org", tmdb_url)
```
